/**
 * version: bitmask parallel with sme parallelism level as normal_parallel.c
**/
    ...
    #pragma omp parallel num_threads(n_thread)
    {
        #pragma omp single nowait
        {
            for (int val = 1; val <= side_length; ++val)
            {
                ...
                #pragma omp task firstprivate(board_copy, board_row_copy, board_col_copy, board_box_copy, unassign_ind, n_unassign, x, y) shared(is_solved)
                {
